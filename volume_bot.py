...
# Fetch futures symbols, fallback from v5 to v2 API if blocked
def get_futures_symbols():
    # Try v5 instruments-info
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if syms:
                ...
                print(f"[DEBUG] v5 API returned empty symbol list, falling back", flush=True)
        else:
            print(f"[ERROR] v5 instruments-info status {resp.status_code}: {resp.text}", flush=True)
    except Exception as e:
        print(f"[ERROR] v5 instruments-info request failed: {e}", flush=True)
    ...
    try:
        ...
        if resp2.status_code == 200:
            ...
            print(f"[ERROR] v2 symbols status {resp2.status_code}: {resp2.text}", flush=True)
    except Exception as e:
...
def get_klines(symbol, interval="60", limit=100):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"[ERROR] kline {symbol} status {resp.status_code}: {resp.text}", flush=True)
    except Exception as e:
...
# Self-ping thread to prevent dyno sleep
def self_ping(port):
    while True:
        try:
